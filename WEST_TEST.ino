// INSTRUCTIONS: RUNNING THIS CODE SHOULD POINT THE ARM PERFECTLY WEST
...
const float THETA1_HOME_DEG = 180 + 110;  // Angle of shoulder when hitting limit (measured 107.8018)
const float THETA2_HOME_DEG = -60;   // Angle of elbow when hitting limit (measured 40.1981)
...
Serial.println("Motors configured. Starting homing sequence...");
  ...
  Serial.println("Homing Complete. Moving to WEST (180 deg) and Straight (0 deg) in 3 seconds...");
  delay(3000); 
  
  // Calculate how many steps it takes to reach 180 degrees
  long west_steps = round(180.0 * STEPS_PER_DEG);
  
  // Command shoulder to 180 degrees (West) and elbow to 0 degrees (Straight)
  stepper2.moveTo(west_steps);
  stepper3.moveTo(0);
}

void loop() {
  // Drive the motors to their targets
  stepper1.run(); 
  stepper2.run(); 
  stepper3.run(); 
}
